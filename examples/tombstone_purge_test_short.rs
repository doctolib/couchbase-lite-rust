...
fn main() {
    ...
    let purge_interval_days = 0.0035; // ~5 minutes
    let wait_minutes = 6;
    set_metadata_purge_interval(purge_interval_days);
    println!("✓ CBS purge interval set to {purge_interval_days} days (~5 minutes)\n");
    ...
    println!("Note: CBS minimum is 1 hour, so tombstone may still exist after this wait.\n");

    for minute in 1..=wait_minutes {
        println!(
            "  [{minute}/{wait_minutes}] Waiting... ({} minutes remaining)",
            wait_minutes - minute
        );
